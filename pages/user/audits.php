      ...
      <ul class="dropdown-menu">
        <?php if (!empty($regulators)) : ?>
          <?php foreach ($regulators as $regulator) : ?>
            <li>
              <a class="dropdown-item" href="create_reports_page?rep=<?php echo htmlentities($regulator['RegulatorName']) ?>&&shortname=<?php echo htmlentities($regulator['ShortName']) ?>">
          ...
          <div class="card accordion-item">
            ...
            <div id="collapse<?php echo $index; ?>" class="accordion-collapse collapse" aria-labelledby="heading<?php echo $index; ?>" data-bs-parent="#ongoingRequestAccordion<?php echo $index; ?>">
              <div class="accordion-body">
                        ...
                        <div class="modal animate__animated animate__bounceIn" id="updateFileModal<?php echo $attachment['id']; ?>" tabindex="-1" aria-labelledby="updateFileModalLabel<?php echo $attachment['id']; ?>" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                              ...
                              <form method="POST" enctype="multipart/form-data" action="replace_file.php">
                ...
                <?php
                // Check if data exists in tblreport_step1 for this request ID
                $stmt_count1 = $dbh->prepare("SELECT COUNT(*) AS count FROM tblreport_step1 WHERE request_id = :request_id");
                $stmt_count1->bindParam(':request_id', $request['id'], PDO::PARAM_INT);
                $stmt_count1->execute();
                $row1 = $stmt_count1->fetch(PDO::FETCH_ASSOC);

                // Check if data exists in tblreport_step2 for this request ID
                $stmt_count2 = $dbh->prepare("SELECT COUNT(*) AS count FROM tblreport_step2 WHERE request_id = :request_id");
                $stmt_count2->bindParam(':request_id', $request['id'], PDO::PARAM_INT);
                $stmt_count2->execute();
                $row2 = $stmt_count2->fetch(PDO::FETCH_ASSOC);

                // Check if data exists in tblreport_step3 for this request ID
                $stmt_count3 = $dbh->prepare("SELECT COUNT(*) AS count FROM tblreport_step3 WHERE request_id = :request_id");
                $stmt_count3->bindParam(':request_id', $request['id'], PDO::PARAM_INT);
                $stmt_count3->execute();
                ...
                <div class="d-flex justify-content-end">
                  ...
                  <?php elseif ($request['status'] == 'approved' && $row1['count'] > 0 && $request['step'] == 1) : ?>
                    <!-- View Report Step 1 -->
                    <a href="view_report_step1.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-primary mt-3 me-2">ពិនិត្យ និងកែប្រែ<i class="bx bx-edit-alt mx-2 me-0"></i></a>
                    <a href="create_reports_page2.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-outline-primary mt-3">បង្កើតសំណើបន្ត<i class="bx bx-chevrons-right mx-2 me-0"></i></a>
                  <?php elseif ($request['status'] == 'approved' && $row2['count'] > 0 && $request['step'] == 2) : ?>
                    <!-- View Report Step 2 -->
                    <a href="view_report_step2.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-primary mt-3 me-2">ពិនិត្យ និងកែប្រែ<i class="bx bx-edit-alt mx-2 me-0"></i></a>
                  ...
                  <?php elseif ($request['status'] == 'completed' && $row3['count'] > 0 && $request['step'] == 3) : ?>
                    <!-- View Report Step 3 -->
                    <a href="view_report_step3.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-primary mt-3 me-2">ពិនិត្យ និងកែប្រែ<i class="bx bx-edit-alt mx-2 me-0"></i></a>
                    <a href="create_reports_final.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-outline-primary mt-3">បង្កើតសំណើបន្ត<i class="bx bx-chevrons-right mx-2 me-0"></i></a>
                  ...
                  <?php elseif ($request['status'] == 'approved' && $request['step'] == 3) : ?>
                    <!-- Make Report Step 2 -->
                    <a href="make_report_step3.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-outline-primary mt-3">បង្កើតរបាយការណ៍<i class="bx bx-chevrons-right mx-2 me-0"></i></a>
                  <?php elseif ($request['status'] == 'pending' && $request['step'] == 2) : ?>
                    <!-- Create Report Step 2 -->
                    <a href="create_reports_page2.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-outline-primary mt-3 disabled">បង្កើតរបាយការណ៍<i class="bx bx-chevrons-right mx-2 me-0"></i></a>
                  <?php elseif ($request['status'] == 'approved' && $request['step'] == 2) : ?>
                    <!-- Make Report Step 2 -->
                    <a href="make_report_step2.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-outline-primary mt-3">បង្កើតរបាយការណ៍<i class="bx bx-chevrons-right mx-2 me-0"></i></a>
                  <?php elseif ($request['status'] == 'approved' && $request['step'] == 1) : ?>
                    <!-- Make Report Step 1 -->
                    <a href="make_report_step1.php?request_id=<?php echo $request['id']; ?>&&shortname=<?php echo $request['shortname'] ?>" class="btn btn-primary mt-3">បង្កើតរបាយការណ៍<i class="bx bx-chevrons-right mx-2 me-0"></i></a>
                  <?php endif; ?>
                </div>
              </div>
            </div>
            ...
            <tbody>
              <?php foreach ($completedAndRejectedRequests as $request) : ?>
                <tr>
                  ...
                  <td>
                    <a href="review_request.php?request_id=<?php echo $request['id']; ?>">Review</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
