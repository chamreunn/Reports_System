...
$stmt = $dbh->prepare("SELECT request_name_1, description_2 FROM tblrequest WHERE id = :id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $stmt = $dbh->prepare("UPDATE tblrequest SET request_name_1 = :request_name, description_3 = :description, status = 'pending',step ='3' WHERE id = :report_id");
  ...
  foreach ($uploaded_files['name'] as $key => $file_name) {
    ...
    if (!in_array($file_name, $old_files)) {
      ...
      $stmt = $dbh->prepare("INSERT INTO tblrequest_attachments (request_id, file_path) VALUES (:request_id, :file_path)");
      $stmt->bindParam(':request_id', $report_id, PDO::PARAM_INT);
      ...
      <div class="card-body">
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="requestName" name="request_name" value="របាយការណ៍សវនកម្ម" required>
          </div>
